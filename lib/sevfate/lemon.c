...
static char* user_templhdrname = NULL;
static void handle_H_option(char *z){
  user_templhdrname = (char *) malloc( lemonStrlen(z)+1 );
  if( user_templhdrname==0 ){
    memory_error();
  }
  lemon_strcpy(user_templhdrname, z);
}
...
int main(int argc, char **argv){
  ...
  static struct s_options options[] = {
    ...
    {OPT_FSTR, "T", (char*)handle_T_option, "Specify a template source file."},
    {OPT_FSTR, "H", (char*)handle_H_option, "Specify a template header file."},
...
PRIVATE char *file_getstem(struct lemon *lemp) {
  ...
  // Copy filename.
  int sz = lemonStrlen(filename) + 5;
  char* name = (char*)malloc( sz );
  lemon_strcpy(name,filename);
...
PRIVATE int translate_code(struct lemon *lemp, struct rule *rp){
  ...
  if( lhsdirect ){
    sprintf(zLhs, "yymsp[%d].minor",1-rp->nrhs);
  }else{
    rc = 1;
          ...
          if( rp->rhsalias[i] && strcmp(cp,rp->rhsalias[i])==0 ){
            if( i==0 && dontUseRhs0 ){
              ...
              ** the token number of X, not the value of X */
              ...
              append_str("yymsp[%d].minor",0,i-rp->nrhs+1, dtnum);
            }
            cp = xp;
            used[i] = 1;
            break;
  ...
  ** saved LHS value now. */
  if( lhsdirect==0 ){
    append_str("  yymsp[%d].minor = ", 0, 1-rp->nrhs, rp->lhs->dtnum);
    append_str(zLhs, 0, 0, 0);
    append_str(";\n", 0, 0, 0);
  }
...
/*
** Set the ".dtnum" field of every terminal and nonterminal
** symbol. Usually this is performed in print_stack_union,
** but as that called has been moved later into PrintHeader,
** seperate this process into another function,
** so it can be called at the correct time.
*/
void build_dt(struct lemon *lemp){
...
void print_stack_union(
  ...
  */
  ...
  fprintf(out,"#define YYMINORTYPE %sTOKENTYPE\n",name); lineno++;
  // Disable the following, as there are not implemented using std::variant.
  ...
  assert(!(lemp->errsym && lemp->errsym->useCnt));
...
void ReportTable(
  ...
  ** The original use of these calls has been moved to ReportHeaders().
  */
  build_dt(lemp);
  ...
  ** empty actions. */
  ...
  for(rp=lemp->rule; rp; rp=rp->next){
    ...
    writeRuleText(out, rp);
    if( rp->neverReduce ){
      fprintf(out, " (NEVER REDUCES) */ yyinvalidcase(yyruleno!=%d, yyruleno);\n",
              rp->iRule); lineno++;
    }else if( rp->doesReduce ){
      fprintf(out, " */ yytestcase(yyruleno==%d);\n", rp->iRule); lineno++;
    }else{
      fprintf(out, " (OPTIMIZED OUT) */ yyinvalidcase(yyruleno!=%d, yyruleno);\n",
              rp->iRule); lineno++;
    }
  }
  fprintf(out,"        break;\n"); lineno++;
...
{
  FILE *out, *in;
  if( user_templhdrname == NULL ) return;
  in = fopen(user_templhdrname, "rb");
  if ( in == NULL ) return;
  ...
  /* Types definitions */
  {
    ...
    print_stack_union(out,lemp,&lineno,0);
  }
  if( lemp->arg && lemp->arg[0] ){
    ...
    fprintf(out,"#define %sARG_PDECL ,[[maybe_unused]]%s\n",name,lemp->arg);  lineno++;
  ...
  if( lemp->ctx && lemp->ctx[0] ){
    int i = lemonStrlen(lemp->ctx);
    while( i>=1 && ISSPACE(lemp->ctx[i-1]) ) i--;
    while( i>=1 && (ISALNUM(lemp->ctx[i-1]) || lemp->ctx[i-1]=='_') ) i--;
    fprintf(out,"#define %sCTX_SDECL %s;\n",name,lemp->ctx);  lineno++;
    fprintf(out,"#define %sCTX_PDECL ,[[maybe_unused]]%s\n",name,lemp->ctx);  lineno++;
  ...
  /* Generate enum definitions for all tokens */
  const char *prefix;
  if( lemp->tokenprefix ) prefix = lemp->tokenprefix;
  else                    prefix = "";
  for(int i=1; i<lemp->nterminal; i++){
    fprintf(out,"    %s%-12s = %3d,\n",prefix,lemp->symbols[i]->name,i);
    lineno++;
  }
  tplt_xfer(lemp->name,in,out,&lineno);
